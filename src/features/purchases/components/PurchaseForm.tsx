...
import { computeSubtotal, computePurchaseTotal } from "../domain/purchaseRules";
import type { PurchaseDetailInput } from "../domain/purchaseRules";
import type { PaymentMethod } from "@/types";
import type { ProductWithStock } from "../../sales/services/productService";
...
interface Props {
  ...
  defaultHeader?: PurchaseFormData;
  defaultDetails?: PurchaseDetailInput[];
  loading?: boolean;
  onSave: (data: PurchaseFormData, details: PurchaseDetailInput[], receive: boolean) => Promise<void>;
}
...
export function PurchaseForm({ products, defaultHeader, defaultDetails, loading, onSave }: Props) {
  const { register, handleSubmit, getValues, formState: { errors } } = useForm<PurchaseFormData>({
  ...
  const [details, setDetails] = useState<PurchaseDetailInput[]>(defaultDetails ?? []);
  const [productId, setProductId] = useState("");
  const [quantity, setQuantity] = useState("1");
  const [unitPrice, setUnitPrice] = useState("");
  const [lineError, setLineError] = useState("");

  const total = computePurchaseTotal(details);
  ...
  const save = (receive: boolean) => {
    const valid = details.length > 0;
    setLineError(valid ? "" : "Agrega al menos un producto");
    if (!valid) return;
    void onSave(getValues(), details, receive);
  };

  return (
    <form onSubmit={handleSubmit((data) => onSave(data, details, false))} className="space-y-4">
      <Input label="Proveedor" {...register("supplier")} error={errors.supplier?.message} placeholder="Ej: Importadora XYZ" />
      ...
      <div className="rounded-2xl bg-zinc-800/40 border border-zinc-800 p-4 space-y-3">
        <p className="text-sm font-semibold text-zinc-300">Detalle de la compra</p>
          ...
          >
            ...
            {products.map((p) => (
              <option key={p.id} value={p.id}>
                {p.code} - {p.name} ({p.color || "sin color"})
              </option>
            ))}
          </select>
          <input
